...
Module Info.
  ...
  Definition source_file := "../digsim-jim/formal/dadd/dadd.c"%string.
...
Definition _dadd : ident := 56%positive.
Definition _main : ident := 57%positive.

Definition f_dadd := {|
  fn_return := tdouble;
  fn_callconv := cc_default;
  fn_params := ((_a, tdouble) :: (_b, tdouble) :: nil);
  fn_vars := nil;
  fn_temps := ((_c, tdouble) :: nil);
  fn_body :=
(Ssequence
  (Sset _c (Ebinop Oadd (Etempvar _a tdouble) (Etempvar _b tdouble) tdouble))
  (Sreturn (Some (Etempvar _c tdouble))))
|}.
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_dadd, Gfun(Internal f_dadd)) :: nil).

Definition public_idents : list ident :=
(_dadd :: ___builtin_debug :: ___builtin_nop ::
